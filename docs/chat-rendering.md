...
## Selecting turns: the selection bubble

**Select** (or a triple-click, or a long press, on a turn) lets turns be picked for **Copy** and **Flag** — in Conversations, Flagged and Analysis alike. What a selection can do lives in one floating bubble over the bottom of the thread (`#selBubble`, `SEL_BUBBLE`, `selBubbleSync`): *N selected · All / None · Copy · Flag · ✕*. Flagged has no Flag; its conversations are flagged already.

- **Not in the header, because the header has no room.** It is one 46 px row already holding the conversation's id, date, context button, Halo Studio link and counts. With the selection's buttons added, under 1100 px the responsive rule wrapped it and put All / None and Cancel on a second line; above it the row's `overflow: hidden` clipped Copy and Flag out of sight. The Analysis headers are fuller still. A header that never wraps is still the rule: under 1100 px its text gives way with an ellipsis.
- **One element, `position: fixed`, placed from the thread's rect** (`selBubblePlace`, again on resize), so a scroll never carries it off and no pane's overflow clips it. Centred with `translate`, not `transform`, because the `pop-in` entrance animates `transform`. Threads get 64 px of bottom padding while it can show, so the last turn can always be scrolled clear of it.
- **The per-view code is unchanged.** The bubble's buttons keep the ids that code already updated in the header (`chatCopySelBtn`, `flaggedSelCount`, `gapFlagSelBtn`, …). `selBubbleSync` only re-renders when the kind changes, so a "✓ Copied!" survives a re-placement.
- **In Analysis the bubble is also the way in.** Its headers get no Select button; while a conversation is shown the bubble waits as a small **Select turns** (`gap-idle`). Analysis threads use the same renderer as Flagged with `turnClick: "onGapTurnClick"`, and Flag calls the same `flag_session` as Conversations, so a turn flagged from a work list lands in Flagged exactly as one flagged in Conversations.
- **A selection belongs to the conversation it was made in.** Opening another conversation, switching the Analysis mode or switching views leaves select mode first (`gapShowThread`, `gapSetMode`, `switchView`). **Esc** leaves it too, unless a dialog is open.
- `exitChatSelectMode` hides Flag as well as Copy; it used to leave Flag on screen after Cancel.
