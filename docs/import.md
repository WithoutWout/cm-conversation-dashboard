...
- **The session context is stored once per distinct value, not once per turn.** `interactions.contexts` was **173 MB of an 824 MB database** — 21% of the whole file — holding 15,404 distinct values across 149,761 rows. CM.com repeats the entire context blob on every turn of a session, and at ~1.2 KB each that is past SQLite's inline-payload limit, so every row spilled onto overflow pages and every scan of `interactions` paid for them. `context_blobs` holds each distinct value once and `interactions.contexts_id` references it: **`interactions` 553 MB → 262 MB, the file 824 MB → 593 MB**, with `answer_index` already added.
  - **`CONTEXTS_EXPR` is the only way any reader reads a context**, because there are four of them and a reader left on the raw column returns empty for every migrated row — which reads as "this conversation had no context" rather than as a bug. Its `COALESCE` fallback to `i.contexts` is permanent, not transitional: the migration nulls the column row by row, so an interrupted run leaves a database with some rows in each shape and both have to read correctly.
  - **The index is on a hash of the JSON, not on the JSON.** A `UNIQUE` constraint on `json` is the obvious spelling and costs a second full copy of every blob: **67 MB against 0.18 MB**, most of the saving handed straight back. Lookup matches on hash *and* json, so a collision — or a change to the hash function — can only cost a duplicate blob row, never a wrong context. That is what makes a non-cryptographic hash correct here. `context_blob_hash` is FNV-1a written out rather than `DefaultHasher`, whose output is explicitly not stable across Rust releases; the value is persisted, so a toolchain upgrade would otherwise stop new imports finding the blobs already stored.
  - **The migration nulls the column as it interns, and that is the half that reclaims anything.** Writing the references alone makes the file larger. It runs in batches of 20,000 rather than one transaction over the whole table, because a single transaction that size holds 173 MB of change in the WAL before any of it lands.
  - **The VACUUM is part of the migration, not left to the user.** Free pages are not free space: without it the file is briefly *larger* and nothing measurable improves, which reads as the upgrade having done nothing. It runs only when rows actually moved, and a failure is logged and shrugged off — the database is correct either way, and **Compact database** can finish the job.
  - **The open overlay says what it is doing**, because ~19 s of "Opening database…" reads as a hang. `open_db_reporting` takes a borrowed reporter — `open_db` keeps its old signature and passes a no-op, so the ~15 tests and the perf harnesses that have no Tauri app still compile — and `set_db_path` emits `db-migrating` from it. The renderer subscribes *before* the invoke, because the first phase fires inside it, and tears the listener down in `finally`, because one left registered would relabel a later open's overlay.
    - **The contexts phase carries a running count, not a percentage.** Knowing how many rows are left means counting rows by their `contexts` value, which is a full scan of the widest column in the database — the very thing the migration exists to stop doing. A live count is honest and free; a progress bar would be neither.
    - Reported per batch, not per row: 20,000 events saying one sentence cost more than the work they describe.
    - `frontend/tests/db-migration-progress.test.js` runs the real `openDbAtPath` against a scripted event stream. It is there because none of the ways this breaks are visible by opening a database — a migration is in flight exactly once per machine. Mutation-checked both ways: moving the subscription after the invoke fails 4 checks, removing the teardown fails 3.
  - **One-time cost is 19-64 s on an 824 MB / 149k-row database** (both index backfills, the interning and the VACUUM together) against ~0.8 s for a normal open afterwards. The spread is the page cache, not the work: 19 s with the file warm, 64 s cold on an 8 GB machine. Quote the range — a first open that takes three times the advertised figure is exactly the case the progress event exists for. Import got slightly *faster*, not slower — narrower rows are less to write; `perf::import_cost` measured 3526 ms against a 4211 ms baseline.
  - `interning_the_contexts_does_not_change_what_any_reader_returns` is the test it rests on: it snapshots every reader before the migration and compares after, asserts the column really was emptied (or the test would pass on a migration that reclaimed nothing), and asserts re-running is a no-op. Verified end to end on a real database as well — **0 of 149,761 contexts and 0 of 18,337 session snapshots differ**.
  - The empty spellings (`''`, `'[]'`, `'null'`, ~1% of rows) are left inline on purpose: four bytes each, and rewriting them is a page write per row to save nothing.
  - `pages` is the same shape of waste at a smaller scale — 24 MB across 2,668 distinct values — and would take the same treatment.
