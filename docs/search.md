...
- **`outputCtxSet` (worker) / `_outputCtxSet` (renderer) is the one reading of one output's conditions**, and callers pass it every output, never only Answers. It drops `any` (not a condition; it matches everyone) and the `escalationGroup` variable, which has chips of its own (see below). `context-filter.test.js` holds the two copies against each other over every output of the real export.
- **`_ctxSets` holds one set per output, empty sets included.** An unconditioned output is `{}`, and that is what lets `not set` pass on it. The old `_hasDefaultAnswer` flag stood in for that empty set, but it only knew about Answers, so an Article whose default output is a route could never be `not set`. It is gone.
  - That also fixed a quiet asymmetry: an Article Answer conditioned *only* on the `escalationGroup` variable was neither pushed as a set nor counted as default, so it matched `not set` on no key at all. The Dialog side already treated it as an empty set.
- **Chips count items, not outputs.** The value chips used to add one per output carrying the value while `any` / `not set` counted items, so a chip could say 12 and return 4. `context-filter.test.js` asserts every chip's count against a real `search` message through the worker, over the real export when present (282 chips on the 2026-08-15 export).
- **With a text query, routes do not count.** `matchArticleCombined` / `matchDialogCombined` still require the text and the condition on the *same* Answer (`_answerItems` stays Answer-only). A route has no text, so it cannot be the output the query matched.
- **A route match explains itself.** `contextRouteMatchSnippet` puts a "Context route" line on the card (target Dialog, node name for a Dialog, and the matching values) when a context filter is on, there is no query, and no Answer satisfies the filter. The Article modal's Linked Dialog section and a Dialog node's routes show each route's conditions as pills, highlighted when they match. An Article with a default Response shows its routes only when one carries context.
- The Collections export already treats a route's context separately (`_colRouteIndex`, `docs/collections.md`). This is the filtering side only.
- **An escalationGroup chip matches the tag *or* the condition.** `escalationGroup` is two things in the export: the metadata tag saying which group a Response belongs to, and a declared context variable an output can be conditioned on. The chips used to read the tag alone, and only on Answers, so `escalationGroup = theater` returned nothing: `theater` is never a tag value, only a condition, on 15 items. `outputEscGroups` (worker) / `_outputEscGroups` (renderer) read both, on every output type, and the item-level aggregate, the per-answer sets (`_answerItems`), `answerPassesContextFilters`, the chip counts and the route snippet all go through it. On the 2026-08-15 export `theater` goes 0 → 15, `attractiepark` 1965 → 2051, and `default` (a condition value, never a tag) gets a chip of its own.
