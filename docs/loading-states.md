...
- **`.mini-spinner` is `inline-block`.** It used to rely on its container being
  flex; placed on a `<span>` in running text (the GAP list, the GAP chat, the
  Segments "Reading the segments…" note) the inline box ignored its width and
  height and drew as a squashed, wobbling sliver. A pane that is loading should
  still use `setPaneLoading`, not a hand-written spinner — the GAP list and chat
  now do.
