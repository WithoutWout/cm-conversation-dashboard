...
- **`conversation_id` is never indexed** (`META_EXCLUDED_KEYS`). It is unique per session, so as a filter it is one chip per conversation matching exactly that conversation — and it was what made the cap bite. Excluded at index time, so it does not cost a row per session either.
