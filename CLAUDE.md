...
- Measured on a 120k-row / 13k-session database with a term matching every row: **never finished → 3.6 s materialized → 31 ms as an `IN`**. `perf::feedback_filter_cost` is that harness and prints `EXPLAIN QUERY PLAN` per case.
